...
import { Schedule, ScheduleWithRosters } from "../types/schedule.types";
import { Roster } from "../../roster/types/roster.types";
import { sampleSchedulesWithRosters } from "@/mocks/data/schedules";
...
export const publish = async (scheduleId: number): Promise<ScheduleWithRosters> => {
  ...
  const newRosters = newSchedule.rosters.map((roster: Roster) => ({
    id: roster.id,
